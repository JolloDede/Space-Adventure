...
public class Draw_Main {
    ...
    public void draw(GraphicsContext g){
        ...
        for (int i = 0; i < Star_Movement.stars.size(); i++){
            g.fillOval(Star_Movement.stars.get(i).getX(), Star_Movement.stars.get(i).getY(), Star_Movement.stars.get(i).getSize(), Star_Movement.stars.get(i).getSize());
        }

        if (!Player.isHit()){
            g.drawImage(IL.iplayer, Player.x , Player.y, Player.width+10, Player.height+10);

            g.setStroke(Color.RED);
            g.strokeRect(Player.x+5, Player.y+5, Player.width-10, Player.height-10);
        }else{
            g.drawImage(IL.iplayer_hit, Player.x , Player.y, Player.width+10, Player.height+10);
        }


        for (Asteroid a: Asteroid_Creation.asteroids){
            if (a.isContainsIridium()){
                g.drawImage(IL.iastroid_i, a.getX(), a.getY(), a.getWidth(), a.getHeight());
            }else{
                g.drawImage(IL.iastroid, a.getX(), a.getY(), a.getWidth(), a.getHeight());
            }
            if (a.isHit()){
